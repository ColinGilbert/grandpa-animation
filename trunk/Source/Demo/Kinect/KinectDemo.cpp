...
HANDLE g_skeletonEvent = NULL;
HANDLE g_videoEvent = NULL;
HANDLE g_videoStream = NULL;

#define DEMO_RIGHT_HAND_COORD  0
...
KinectCharacter* g_character = NULL;
...
IDirect3DVertexBuffer9* g_pVbVideo = NULL;
...
float g_aVertexVideo[] = 
{
	0, 3.0f, 0, 0.0f, 1.0f,
	 FLOOR_SIZE, 3.0f, 0, 1.0f, 1.0f,
	0, 3.0f, FLOOR_SIZE * 0.75f, 0.0f, 0.0f,
	 FLOOR_SIZE, 3.0f, 0, 1.0f, 1.0f,
	 FLOOR_SIZE, 3.0f,  FLOOR_SIZE * 0.75f, 1.0f, 0.0f,
	0, 3.0f,  FLOOR_SIZE * 0.75f, 0.0f, 0.0f
	//-FLOOR_SIZE, 0, 2.0f, 0.0f, 1.0f,
	// 0, 0, 2.0f, 1.0f, 1.0f,
	//-FLOOR_SIZE, 0, 0.0f, 0.0f, 0.0f,
	//-FLOOR_SIZE, 0, 0.0f, 1.0f, 1.0f,
	// 0, 0, 2.0f, 1.0f, 0.0f,
	// 0, 0, 0.0f, 0.0f, 0.0f
};
...
IDirect3DTexture9* g_videoTexture = NULL;

//BYTE* g_videoBuffer = NULL;
...
INT WINAPI wWinMain(HINSTANCE, HINSTANCE, LPWSTR, int)
{
	...
	if (g_videoEvent != NULL && g_videoEvent != INVALID_HANDLE_VALUE)
	{
		CloseHandle(g_videoEvent);
		g_videoEvent = NULL;
	}
	if (g_skeletonEvent != NULL && g_skeletonEvent != INVALID_HANDLE_VALUE)
	{
		CloseHandle(g_skeletonEvent);
		g_skeletonEvent = NULL;
	}

	//if (g_videoBuffer != NULL)
	//{
	//	delete[] g_videoBuffer;
	//	g_videoBuffer = NULL;
	//}
...
void InitApp()
{
    ...
    if (FAILED(hr))
	{
		::MessageBox(NULL, L"NuiInitialize failed, Please connect your Kinect and retry.", L"Error", MB_OK | MB_ICONERROR);
		exit(0);
	}

	g_skeletonEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
	g_videoEvent = CreateEvent(NULL, TRUE, FALSE, NULL);

	hr = NuiSkeletonTrackingEnable( g_skeletonEvent, 0 );

	hr = NuiImageStreamOpen(NUI_IMAGE_TYPE_COLOR, NUI_IMAGE_RESOLUTION_640x480, 0, 2, g_videoEvent,
        &g_videoStream );

	//g_videoBuffer = new BYTE[640 * 480 * 4];
...
HRESULT CALLBACK OnCreateDevice(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
...
#else
	g_camera.setViewParams(grp::Vector3(0.5f, 1.0f, -2.0f),
	...
	g_character = new KinectCharacter(L"Test/warrior.gmd", pd3dDevice);
							   ...
							   const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	...
	V_RETURN(::D3DXCreateTextureFromFile(pd3dDevice, L"Test/floor.dds", &g_floorTexture));
	V_RETURN(::D3DXCreateTexture(pd3dDevice, 640, 480, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &g_videoTexture));
...
void processSkeletonEvent()
{
	...
	for (i = 0; i < NUI_SKELETON_COUNT; i++)
	{
		...
		{
			grp::Vector3 jointPos(data.SkeletonPositions[j].x * 1.1f,
									data.SkeletonPositions[j].z * 1.1f,
									data.SkeletonPositions[j].y * 1.1f + 1.0f);
			g_character->setKinectJointPos(j, jointPos);
		}
		break;
	}
	g_character->enableKinectControl(i != NUI_SKELETON_COUNT);
}

void processVideoEvent()
{
	const NUI_IMAGE_FRAME * imageFrame = NULL;
	HRESULT hr = NuiImageStreamGetNextFrame(g_videoStream, 0, &imageFrame);
	if (FAILED(hr))
	{
		return;
	}
	...
	if (lockedRect.Pitch != 0)
	{
		...
		D3DLOCKED_RECT lr;
		hr = g_videoTexture->LockRect(0, &lr, NULL, 0);
		if (FAILED(hr))
		{
			int a = 0;
		}
		memcpy(lr.pBits, buffer, 640*480*4);
		g_videoTexture->UnlockRect(0);
		//g_videoTexture->LockRect(NULL, NULL, 0);
		//memcpy(g_videoBuffer, buffer, 640 * 480 * 4);
	}
	NuiImageStreamReleaseFrame(g_videoStream, imageFrame);
}
...
void CALLBACK OnFrameMove(double fTime, float fElapsedTime, void* pUserContext)
{
	...
	HANDLE events[2];
	events[0] = g_skeletonEvent;
	events[1] = g_videoEvent;

	int eventIndex = WaitForMultipleObjects(sizeof(events) / sizeof(events[0]), events, FALSE, 20);
...
void CALLBACK OnFrameRender(IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
	...
	V(g_pVbVideo->Lock(0, 0, (void**)&pVertex, 0));
	memcpy(pVertex, g_aVertexVideo, sizeof(g_aVertexVideo));	
	V(g_pVbVideo->Unlock());
	...
	if(SUCCEEDED(pd3dDevice->BeginScene()))
	{
		...
		V(g_pEffect->SetTexture("g_texDiffuse", g_floorTexture));
		...
		V(pd3dDevice->SetStreamSource(0, g_pVbVideo, 0, 20));
		V(g_pEffect->SetTexture("g_texDiffuse", g_videoTexture));
		V(g_pEffect->Begin(&cPasses, 0));
		for (iPass = 0; iPass < cPasses; iPass++)
		{
			V(g_pEffect->BeginPass(iPass));
	...
	//D3DLOCKED_RECT lr;
	//bb->LockRect(&lr, NULL, D3DLOCK_NOSYSLOCK);
}
...
void CALLBACK OnLostDevice(void* pUserContext)
{
	...
	SAFE_RELEASE(g_floorTexture);
	SAFE_RELEASE(g_videoTexture);
	
	SAFE_RELEASE(g_pVbFloor);
	SAFE_RELEASE(g_pVbVideo);
