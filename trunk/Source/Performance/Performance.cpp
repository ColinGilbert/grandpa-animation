...
static long long g_i64CpuFreq = 2000000000;	//默认CPU指令频率
...
struct PERF_NODE_DATA
{
	...
	long long	iTotalTime;		//总开销时间(cpu周期数)
};
///////////////////////////////////////////////////////////////////////////////////////////////////
									...
									)
{
	...
	int iFrameSizeOld	= sizeof( int ) * iUserDataSize		//用户数据
						+ sizeof( int )						//采样帧数
						+ sizeof( __time64_t )					//时间（1970年来的秒数）
						+ sizeOfNodeData() * iTotalNodeOld;	//采样数据
	int iFrameSizeNew	= sizeof( int ) * iUserDataSize		//用户数据
						+ sizeof( int )						//采样帧数
						+ sizeof( __time64_t )					//时间（1970年来的秒数）
						+ sizeOfNodeData() * m_iTotalNode;	//采样数据
	assert( iFrameSizeNew > iFrameSizeOld );
	//第一帧数据偏移
	int iPosOld	= sizeOfHeader()		//文件头
				+ sizeOfNodeHeader() * iTotalNodeOld	//采样节点头
				+ MAX_NODE_NAME_LEN * iUserDataSize;		//用户数据名
	int iPosNew	= sizeOfHeader()		//文件头
				+ sizeOfNodeHeader() * m_iTotalNode	//采样节点头
				+ MAX_NODE_NAME_LEN * iUserDataSize;		//用户数据名
...
bool PerfManager::SaveDataFrame( const char* pPath, int* pUserData, const char** pUserDataName, int iUserDataSize )
{
	...
	int iBufferSize = sizeof( int ) * iUserDataSize	//用户数据
					+ sizeof( int )			//采样帧数
					+ sizeof( __time64_t )		//时间（1970年来的秒数）
					+ sizeOfNodeData() * m_iTotalNode;	//节点数据
	char *pBuffer = new char[iBufferSize];
	char *pCur = pBuffer;
	//用户数据
	if ( pUserData != NULL )
...
bool PerfManager::OpenDataFile( const char* pPath, int* pTotalDataFrame, int* pUserDataSize )
{
	...
	int iFrameSize	= sizeof( int ) * fh.iUserDataSize	//用户数据
					+ sizeof( int )		//采样帧数
					+ sizeof( __time64_t )	//时间（1970年来的秒数）
					+ sizeOfNodeData() * fh.iNumNode;	//采样数据

	int iHeaderSize = sizeOfHeader()		//文件头
					+ sizeOfNodeHeader() * fh.iNumNode	//采样节点头
...
{
	return	sizeof(unsigned int)//	dwFlag;			//文件标识
		+	sizeof(unsigned int)//	dwVersion;		//版本
		+	sizeof(long long)//		i64CPUFreq;		//cpu主频
		+	sizeof(int)//			iNumNode;		//采样节点总数
		+	sizeof(int)//			iUserDataSize;	//用户数据总数
		+	sizeof(unsigned int)//	dwReserve1;		//备用
...
{
	return	MAX_NODE_NAME_LEN	//节点名称
		+	sizeof(int)//		iParent;				//父节点序号
		+	sizeof(int)//		iSibling;				//下一个兄弟节点序号
		+	sizeof(int);//		iChild;					//子节点序号
}
...
{
	return	sizeof(int)//			iTotalCalls;	//总调用次数
		+	sizeof(long long);//		iTotalTime;		//总开销时间(cpu周期数)
}
