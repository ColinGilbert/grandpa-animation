...
enum ControlId
{
	IDC_STATIC_WEIGHT =	10,
	IDC_SLIDER_WEIGHT,
	IDC_STATIC_TIMESCALE,
	IDC_SLIDER_TIMESCALE,
	IDC_STATIC_ARM,
	IDC_SLIDER_ARM,
	IDC_STATIC_LEG,
	IDC_SLIDER_LEG,
...
void set_bone_scale(grp::ISkeleton* skeleton, const wchar_t* bone_name, float scale)
{
	assert(skeleton != NULL);
	assert(bone_name != NULL);

	grp::IBone* bone = skeleton->getBoneByName(bone_name);
	if (bone == NULL)
	{
		return;
	}
	grp::Matrix m = grp::Matrix::IDENTITY;
	m._22 = m._33 = scale;
	bone->setAbsoluteTransform(m * bone->getAbsoluteTransform());
}

class BoneController : public grp::ISkeletonCallback
{
public:
...
BoneController g_boneController;
...
HRESULT CALLBACK OnCreateDevice(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	...
	if (skeleton != NULL)
	{
		skeleton->setCallback(&g_boneController);
	}
	//g_skeleton->setCallback(
